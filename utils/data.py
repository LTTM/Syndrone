...
class SelmaDrones(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for k,v in self.idmap.items():
            mlb[sem==k] = cm[v] if self.coarselabels else v
    ...
    def to_pytorch(self, rgb, dth, mlb):
        if self.modality == 'all':
            return torch.from_numpy(rgb).permute(2,0,1), torch.from_numpy(dth), torch.from_numpy(mlb)
...
def coarse_mapping():
    fine2coarse_idx = [4, 4, 5, 5, 0, 0, 0, 1, 4, 5, -1, 0, 4, 0, 5, 5, 5, 5, 6, 1, 2, 2, 3, 3, 3, 3, 3, 3]
    coarse_labelnames = ["Road", "Nature", "Person", "Vehicle", "Construction", "Obstacle", "Water"]
    return fine2coarse_idx, coarse_labelnames

if __name__ == "__main__":
    from matplotlib import pyplot as plt

    d = SelmaDrones("../../Datasets/selmadrones",
                    "../../Datasets/selmadrones",
                    coarselabels=True)
    ...
    for (rgb, mlb), _ in d:
        ...
        axs[1].imshow(cmap[mlb]) #d.color_label(mlb))
        plt.show()